...
angular.module('faradayApp')
    .directive('customFieldsVulnDb', [function () {
        return {
            restrict: 'E',
            scope: true,
            replace: true,
            template: '<div>\
                            <div ng-if="cf.field_type === \'str\'" ng-init="isEditable = true"> \n\
                                ...
                                <div class="form-group"> \n\
                                    ...
                                    <textarea class="form-control" rows="5" id="vuln-desc" name="desc" ng-show="isEditable === true" \n\
                                        ng-model="data.customfields[cf.field_name]" ng-bind-html="data.customfields[cf.field_name] | markdown" \n\
                                        style="margin: 0 2px 0 0;" ng-blur="isEditable = isEditable.length==0 || !data.customfields[cf.field_name]" autofocus> \n\
                                    </textarea> \n\
                                    <div class="col-md-12" ng-cloak ng-show="data.customfields[cf.field_name].length > 0 && isEditable === false"> \n\
                                        <div class="markdown-preview" style="height: 100px;!important;" ng-bind-html="data.customfields[cf.field_name] | markdown" ng-dblclick="isEditable = true">{{data.customfields[cf.field_name] | markdown}}</div> \n\
                                    </div> \n\
                                </div> \n\
                            </div> \n\
                                ...
                                <div class="tab-pane-header">{{cf.field_display_name}}</div> \n\
                                    <div class="form-group"> \n\
                                        ...
                                        <input type="text" class="form-control input-sm" id="{{cf.field_name}}" name="{{cf.field_name}}" \n\
                                               ...
                                               ng-model="data.customfields[cf.field_name]" check-custom-type="{{cf.field_type}}" \n\
                            ...
                            <div ng-if="cf.field_type === \'list\'"> \n\
                                <div class="tab-pane-header">{{cf.field_display_name}}</div> \n\
                                <div class="form-group"  ng-class="data.customfields[cf.field_name].length > 0 ? \'no-margin-bottom\' : \'\'"> \n\
                                ...
                                <div class="reference" ng-repeat="item in data.customfields[cf.field_name] track by $index" ng-class="{\'last-item-field\':$last}" ng-if="cf.field_type === \'list\'"> \n\
                                    <div class="input-group margin-bottom-sm"> \n\
                                        ...
                                        <input ng-if="!item.value" type="text" class="form-control input-sm" id="vuln-refs-create" placeholder="{{cf.field_display_name}}" \n\
                                               ng-model="item" \n\
                                               role="button" readonly/> \n\
                                        <span class="input-group-addon cursor" ng-click="data.customfields[cf.field_name].splice($index, 1)"> \n\
            ...
            link: function (scope, element, attrs) {
                ...
                scope.newValueField = function (valueField) {
                    if (valueField !== "" && valueField !== undefined) {
                        if(scope.data.customfields[scope.cf.field_name] == null )
                            scope.data.customfields[scope.cf.field_name] = [];

                        // we need to check if the ref already exists
                        if (scope.data.customfields[scope.cf.field_name].filter(function(field) {return field.value === valueField}).length === 0) {
                            scope.data.customfields[scope.cf.field_name].push({value: valueField});
                            scope.valueField = "";
                        }
