...
#-------------------------------------------------------------------------------
#TODO: may be we should do some refactoring and rename this to something
# like HostsController, because it doesn't control all the model.
# MainApplication is doing that

#TODO: we need to change all public async methods to add a parameter or something
# to be able to determine who did it. This is needed for History logging

class ModelController(threading.Thread):
    """
    The model controller handles all the changes in the application model, like
    hosts, services, etc
    This controller act as a bridge between the widgets in the GUI and model
    objects.
    Implemented like a MVP (Model-View-Presenter) design pattern
    It acts as a daemon thread like in a producer-consumer scenary
    The controller is a consumer, because it gets new hosts from a queue
    The producers are other components which create new hosts discovered (commonly
    done by an API)
    """
    ...
    def __init__(self, security_manager ):
        ...
        #TODO: should we have any other list with objects?
        #TODO: if we work with events then this list may not be needed
        # list of widgets controller must notify when model changes
        self._widgets       = []
        ...
        # sync api request flag. This flag is used to let the model know there's
        # some other object trying to use a sync api, and it should give
        # priority to that and stop processing the queue
        ...
        #XXX: clear to start from scratch .ONLY FOR DEMO!!!!!!!
        if os.path.exists(CONF.getHsactionsPath()):
            os.remove(CONF.getHsactionsPath())
    ...
    def __getattr__( self, name):
        api.devlog("ModelObject attribute to refactor: %s" % name)
    ...
    def _logAction(self, d1, d2, d3 = ""):
        return
        #XXX: temporal for demo purposes
        # logs a line in a txt file
        line = "|".join(["%s" % datetime.datetime.now(), d1, d2, d3])
        api.devlog("logging action to file %s" % CONF.getHsActionsPath())
        f = open(CONF.getHsActionsPath(),"a")
        f.write("%s\n" % line)
        api.devlog("action = %s" % line)
        f.close()
    ...
    def _processAction(self, action, parameters, sync=False):
        ...
        api.devlog("_processAction - %s - parameters = %s" % (action, str(parameters)))
        ...
        if res: # notify only if action was done successfuly
            ...
            api.devlog("Action code %d failed. Parameters = %s" % (action, str(parameters)))
    ...
    def resolveConflicts(self):
        self._notifyConflictResolution(self.getConflicts())
    ...
    def resolveConflict(self, conflict, kwargs):
        if conflict.resolve(kwargs):
            ...
            self._notifyConflictsUpdated(-1)
    ...
    def removeConflictsByObject(self, obj):
        ...
        self._notifyConflictsUpdated(-len(obj.getUpdates()))
    
    def _loadCategories(self, path):
        #TODO:
        pass
    ...
    def _main(self):
        ...
        while True:
            ...
            #XXX: we have to persist information somehow
            # self.persistData()

            # first we check if there is a sync api request
            # or if the model is being saved/sync'ed
            # or if we have pending duplicated hosts that need to be merged by the user
            if not self._sync_api_request and  \
                not self._saving_model_flag:

                self.processAction()

            else:
                # there is some object requesting for a sync api so we sleep the
                # thread execution for a moment to let others work
    ...
    def processAction(self):
        ...
        try:
           ...
           api.log("something strange happened... unhandled exception?", "ERROR")
           api.log(traceback.format_exc(), "ERROR")

    def registerWidget(self, widget):
        """
        Registers a widget to be notified when the model changed
        The widget MUST implement a customeEvent handler able to
        process an ModelObjectUpdateEvent
        taking as parameters an action and a list of parameters
        """
    ...
    def deregisterWidget(self, widget):
        self._widgets_lock.acquire()
    ...
    def _notifyWidgets(self, event):
        self._widgets_lock.acquire()
        #TODO: be carefull with deadlocks or passing a list of hosts, because
        # it will be a reference that could change while iterating over it
        for w in self._widgets:
            guiapi.postCustomEvent(event, w)
        self._widgets_lock.release()

    def _notifyModelUpdated(self, *args, **kwargs):
        #TODO: be carefull with deadlocks or passing a list of hosts, because
        # it will be a reference that could change while iterating over it
        #print "LEN HOSTS CONTROLLER: %s" % str(len(self.getAllHosts()))
        #print "LEN HOSTS WORKSPACE: %s" % str(len(self.getWorkspace().getContainee()))
        self._notifyWidgets(ModelObjectUpdateEvent(self.getAllHosts()))
    
    def _notifyRenameRoot(self, name):
        self._notifyWidgets(RenameHostsRootCustomEvent(name))

    def _notifyWorkspaceChanged(self, workspace):
        self._notifyWidgets(WorkspaceChangedCustomEvent(workspace))

    def _notifyConflictsUpdated(self, vulns_changed):
        self._notifyWidgets(ConflictUpdatedCustomEvent(vulns_changed))

    def _notifyConflictResolution(self, conflicts):
        self._notifyWidgets(ResolveConflictsCustomEvent(conflicts))
            
    def _notifyDiffHost(self, hostLeft, hostRight):
        self._notifyWidgets(DiffHostsCustomEvent(hostLeft, hostRight))

    def _notifyWorkspaceConnectionLost(self):
        guiapi.postCustomEvent((ShowPopupCustomEvent("Couchdb Connection lost. Defaulting to memory. Fix network and try again in 5 minutes.")))

    def _notifyNoVisualizationAvailable(self):
        guiapi.postCustomEvent((ShowPopupCustomEvent("You need Couchdb to run visualizations, sorry")))


    def _notifySyncError(self):
        #self._notifyWidgets(ShowPopupCustomEvent("Sync Failed!", "ERROR"))
        guiapi.postCustomEvent((ShowPopupCustomEvent("Sync Failed! \nYou should check if there are some conflicts to resolve")))
    ...
    # also we need some "move" api to be used when drag & drop exists in the host browser
...
#        self.sync_lock()
#        self.__delCategory(category, recursive)
#        self.sync_unlock()
    ...
    def editInterfaceSYNC(self, interface, name, description, hostnames, mac, ipv4, ipv6, network_segment, 
                  amount_ports_opened, amount_ports_closed, amount_ports_filtered, owned):
        """
        SYNC API
        Modifies an interface from model
        """
        self._processAction(modelactions.EDITINTERFACE, [interface, name, description, hostnames, mac, ipv4, ipv6, network_segment, 
                  amount_ports_opened, amount_ports_closed, amount_ports_filtered, owned], sync=True)
    
    def __editInterface(self, interface, name, description, hostnames, mac, ipv4, ipv6, network_segment, 
                  amount_ports_opened, amount_ports_closed, amount_ports_filtered, owned):
        res = False
        if interface is not None:
            interface.updateAttributes(name, description, hostnames, mac, ipv4, ipv6, network_segment, 
                   amount_ports_opened, amount_ports_closed, amount_ports_filtered, owned)
    ...
    def delServiceFromInterfaceASYNC(self, host, interface, service):
        ...
        Interface parameter can be "ALL"
        """
    ...
    def __delServiceFromInterface(self, host_id, interface_id=None, service_id=None):
        res = False
        api.devlog("ModelController.__delServiceFromInterface(%s, %s, %s)" % (host_id, interface_id, service_id))
        host = self._getValueByID("_hosts", host_id)
        if host is not None:
    ...
    def __delServiceFromApplication(self, host_id, application_id=None, service_id=None):
        res = False
        api.devlog("ModelController.__delService(%s, %s, %s)" % (host_id, application_id, service_id))
        host = self._getValueByID("_hosts", host_id)
        if host is not None:
    ...
    def __delService(self, host_id, service_id=None):
        res = False
        api.devlog("ModelController.__delService(%s, %s)" % (host_id, service_id))
        host = self._getValueByID("_hosts", host_id)
        if host is not None:
    ...
    def __editService(self, service, name=None, description=None, protocol=None, ports=None, 
    ...
    def __delVulnerability(self, action, host_name, item_name, vuln_id):

        res = False
        #self.__acquire_host_lock()
        # DO NOT USE self.getHost because it will cause a deadlock
        # if interface name is ALL then we delete the service from the whole host
        host = self._getValueByID("_hosts", host_name)
        if host is not None:
            if action == modelactions.DELVULNHOST:
                res = host.delVuln(vuln_id)
            else:

                if action == modelactions.DELVULNAPP:
                    _getOne = host.getApplication
                    _getAll = host.getAllApplications
                    _delItem = host.delApplication
                elif action == modelactions.DELVULNINT:
                    _getOne = host.getInterface
                    _getAll = host.getAllInterfaces
                    _delItem = host.delInterface
                elif action == modelactions.DELVULNSRV:
                ...
                if item_name != "ALL":
                    ...
                    if item is not None:
                        res = item.delVuln(vuln_id)
                else:
                    # remove from all interfaces
                    for item in _getAll():
                        res = item.delVuln(service.getID())
    ...
    def editVulnWebSYNC(self, vuln, name, desc, website, path, refs, severity, request, response,
                method,pname, params,query,category):
        self._processAction(modelactions.EDITVULN, [vuln, name, desc, website, path, refs, severity, request, response,
                method,pname, params,query,category], sync=True)

    def editVulnWebASYNC(self, vuln, name, desc, website, path, refs, severity, request, response,
                method,pname, params,query,category):
        self.__addPendingAction(modelactions.EDITVULN, [vuln, name, desc, website, path, refs, severity, request, response,
                method,pname, params,query,category])
    ...
    def __addNoteToHost(self, host_id, note=None):
        ...
        if host is not None:
            ...
            api.devlog("__addNoteToHost failed. Hostname: %s not found" % host_id)
        return res
    ...
    def clearHosts(self, use_lock=True):
        """
        Clears the hosts saved in the model. This does not delete hosts from
        workspaces or filesystem. Do not confuse this with deleteAllHosts
        """
        api.devlog("model controller clearHosts called")
        # this method should be called when closing the current workspace
        if use_lock: self.__acquire_host_lock()
        self._hosts.clear()
        self._widgets_lock.acquire()
        for w in self._widgets:
            event = ClearHostsCustomEvent()
            guiapi.postCustomEvent(event, w)
        self._widgets_lock.release()
        if use_lock: self.__release_host_lock()
    
    def deleteAllHosts(self):
        """
        Be carefull when calling this method!
        This will delete all hosts from the model and the workspace (as well
        as the filesystem). This will delete all the contents and this will
        be syncronized!!
        """
        #TODO: review this...
        # should we allow this to everyone?
        pass

    def clearWidgets(self):
        #XXX: is this really usefull??
        self._widgets_lock.acquire()
        del self._widgets[:]
        self._widgets_lock.release()
    ...
    def setWorkspace(self, workspace):
        ...
        self._notifyWorkspaceChanged(self._workspace)
        self._notifyModelUpdated()
