...
class DnsmapPlugin(core.PluginBase):
    ...
    def __init__(self):
        ...
        global current_path
        self.output_path = os.path.join(self.data_path,"dnsmap_output-%s.txt" % self._rid)
    ...
    def parseOutputString(self, output, debug = False):
        ...
        if debug:
            parser = DnsmapParser(self.output_path)
        else:
    ...
    def processCommandString(self, username, current_path, command_string):
        ...
        if arg_match is None:
            return "%s -c %s \n" % (command_string,self.output_path)
        else:
            return re.sub(arg_match.group(1),
                          r"-c %s" % self.output_path,
                          command_string)    
