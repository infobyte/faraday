        ...
        for host in self.validHosts(parser.hosts):
            ...
            if self._isIPV4(str(host.address)):
                i_id = self.createAndAddInterface(
                    h_id, name=host.address, ipv4_address=host.address, hostname_resolution=hostname)
            else:
                i_id = self.createAndAddInterface(
                    h_id, name=host.address, ipv6_address=host.address, hostname_resolution=hostname)

            if host.type == "info":
                s_id = self.createAndAddServiceToInterface(
                    h_id, i_id, "domain", protocol="tcp", ports=["53"], status="open")
