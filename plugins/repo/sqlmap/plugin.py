...
class SqlmapPlugin(PluginTerminalOutput):
    ...
    def parseOutputString(self, output, debug=False):
        ...
        i_id = self.createAndAddInterface(
            h_id, name=self.ip, ipv4_address=self.ip, hostname_resolution=self.hostname)
        ...
        if absFilePaths:
            n_id2 = self.createAndAddNoteToService(
                h_id, s_id2, "sqlmap.absFilePaths", str(absFilePaths))
        if tables:
            n_id2 = self.createAndAddNoteToService(
                h_id, s_id2, "sqlmap.brutetables", str(tables))
        if columns:
            n_id2 = self.createAndAddNoteToService(
                h_id, s_id2, "sqlmap.brutecolumns", str(columns))
        if xpCmdshellAvailable:
            n_id2 = self.createAndAddNoteToService(
                h_id, s_id2, "sqlmap.xpCmdshellAvailable", str(xpCmdshellAvailable))
        ...
        for inj in self.hashDBRetrieve(self.HASHDB_KEYS.KB_INJECTIONS, True, db) or []:
            ...
            if user:
                n_id2 = self.createAndAddNoteToService(
                    h_id, s_id2, "db.user", user)
            if dbname:
                n_id2 = self.createAndAddNoteToService(
                    h_id, s_id2, "db.name", dbname)
            if hostname:
                n_id2 = self.createAndAddNoteToService(
                    h_id, s_id2, "db.hostname", hostname)
            if dbversion:
                n_id2 = self.createAndAddNoteToService(
                    h_id, s_id2, "db.version", dbversion)
            if dbs:
                n_id2 = self.createAndAddNoteToService(
                    h_id, s_id2, "db.databases", str(dbs))
