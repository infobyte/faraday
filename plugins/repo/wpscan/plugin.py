...
__author__ = "Joaquin L. Pereyra | Federico Fernandez | Jorge L. Gonzàlez Iznaga"
__copyright__ = "Copyright (c) 2018, Infobyte LLC"
...
class WPscanParser(object):
    """
    The objective of this class is to parse an output generated by the wpscan tool.
    @param wpscan output
    """

    def __init__(self, output):
        self.wp_url = re.search(r"URL: ((http[s]?)\:\/\/([\w\.]+)[.\S]+)", output)
        self.service, self.base_url = self.get_service_and_url(output)
        self.port = self.get_port()
        self.host_ip = socket.gethostbyname_ex(self.base_url)[2][0]
        self.potential_vulns = re.findall(r"(\[\!\].*)", output)
        self.themes = {}
        self.plugins = {}
        self.wpversion = ''
    ...
    def get_service_and_url(self, output):
        """ 
...
class WPScanPlugin(core.PluginBase):
    ...
    def parseOutputString(self, output, debug=False):
        ...
        parser = WPscanParser(output)

        host_id = self.createAndAddHost(parser.host_ip)
        interface_id = self.createAndAddInterface(host_id, parser.host_ip,
                                                  ipv4_address=parser.host_ip,
                                                  hostname_resolution=parser.base_url)

        service_id = self.createAndAddServiceToInterface(host_id, interface_id,
                                                         parser.service, "tcp", ports=[parser.port])

        for potential_vuln in parser.potential_vulns:
            vuln_name, severity = self.get_name_and_severity(potential_vuln)
            if vuln_name is not None:
                vuln = potential_vuln  # they grow up so fast
                path = self.get_path_from_vuln(vuln)
                self.createAndAddVulnWebToService(host_id, service_id,
                                                  name=vuln_name,
                                                  website=parser.base_url,
                                                  path=path, severity=severity)

        if len(parser.plugins) > 0:
            self.add_themes_or_plugins_vulns('plugins.json', parser.plugins, host_id, service_id, parser.base_url,
                                             parser.wp_url.group(1),
                                             'plugins')
        if len(parser.wpversion) > 0:
            self.add_wp_vulns('wordpresses.json', parser.wpversion, host_id, service_id, parser.base_url)
        if len(parser.themes) > 0:
            self.add_themes_or_plugins_vulns('themes.json', parser.themes, host_id, service_id, parser.base_url,
                                             parser.wp_url.group(1),
                                             'themes')
    ...
    def get_name_and_severity(self, potential_vuln):
    ...
    def get_path_from_vuln(self, vuln):
    ...
    def add_themes_or_plugins_vulns(self, db, dic, host_id, serv_id, domain, wp_url, name):
                ...
                for title in dic[p]:
                    for vuln in j[p]['vulnerabilities']:  # iter vulnerabilities
                        if vuln['title'] == title:  # if output title is equal
                            ...
                            self.createAndAddVulnWebToService(host_id, serv_id, title, severity=risk, website=domain,
                                                              ref=refs, path=location)

    def add_wp_vulns(self, db, version, host_id, serv_id, domain):
        with open(self.wpPath + '/data/' + db, "r") as data:
            j = json.load(data)
