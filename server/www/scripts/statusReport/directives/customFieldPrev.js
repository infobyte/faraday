...
angular.module('faradayApp')
    .directive('customFieldPrev', ['vulnsManager', function (vulnsManager) {
        return {
            ...
            scope: false,
            replace: true,
            template: '<div><div class="tab-pane-header"><i class="fa fa-spinner fa-spin" ng-show="isUpdatingVuln === true && fieldToEdit === cf.field_display_name"></i>    {{cf.field_display_name}}</div> \n\
                            <div class="form-group" ng-if="cf.field_type !== \'list\'"> \n\
                                ...
                                <input type="text" class="form-control input-sm" id="{{cf.field_name}}" name="{{cf.field_name}}" \n\
                                       ...
                                       ng-focus="activeEditPreview(cf.field_display_name)" \
                                       ng-blur="processToEditPreview(false)"\
                                       ng-model="lastClickedVuln.custom_fields[cf.field_display_name]" check-custom-type="{{cf.field_type}}" \n\
                            ...
                            <div class="col-md-12 reference last-item-field" ng-repeat="item in lastClickedVuln.custom_fields[cf.field_display_name] track by $index" ng-class="{\'last-item-field\':$last}" ng-if="cf.field_type === \'list\'"> \n\
            ...
            link: function (scope, element, attrs) {
                scope.newValueField = function (valueField) {
                    if (valueField !== "" && valueField !== undefined) {
                        if(scope.lastClickedVuln.custom_fields[scope.cf.field_display_name] === null )
                            scope.lastClickedVuln.custom_fields[scope.cf.field_display_name] = [];

                        // we need to check if the ref already exists
                        if (scope.lastClickedVuln.custom_fields[scope.cf.field_display_name].filter(function(field) {return field.value === valueField}).length === 0) {
                            scope.lastClickedVuln.custom_fields[scope.cf.field_display_name].push({value: valueField});
                            scope.valueField = "";
                        }
                        angular.element('#'+scope.cf.field_name).val("");

                        scope.fieldToEdit = scope.cf.field_display_name;
                ...
                scope.removeValueField = function (index) {
                    scope.fieldToEdit = scope.cf.field_display_name;
                    scope.lastClickedVuln.custom_fields[scope.cf.field_display_name].splice(index, 1);
