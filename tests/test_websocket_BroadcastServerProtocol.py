...
class TestWebsockerBroadcastServerProtocol():
    ...
    def test_join_agent_message_with_invalid_token_fails(self, session, test_client):
    ...
    def test_join_agent_message_without_token_fails(self, session, test_client):
        factory = WorkspaceServerFactory('ws://127.0.0.1')
        proto = factory.buildProtocol(('127.0.0.1', 0))
        message = '{"action": "JOIN_AGENT"}'
        assert not proto.onMessage(message, False)

    def test_join_agent_message_with_valid_token(self, session, test_client):
    ...
    def test_leave_agent_happy_path(self, session, test_client):
        token = _join_agent(test_client, session)
        factory = WorkspaceServerFactory('ws://127.0.0.1')
        proto = factory.buildProtocol(('127.0.0.1', 0))
        message = '{{"action": "JOIN_AGENT", "token": "{}" }}'.format(token)
    ...
    def test_leave_agent_without_token_fails(self, session, test_client):
        token = _join_agent(test_client, session)
        factory = WorkspaceServerFactory('ws://127.0.0.1')
        proto = factory.buildProtocol(('127.0.0.1', 0))
        message = '{{"action": "JOIN_AGENT", "token": "{}" }}'.format(token)
    ...
    def test_leave_agent_with_invalid_token_fails(self, session, test_client):
        token = _join_agent(test_client, session)
        factory = WorkspaceServerFactory('ws://127.0.0.1')
        proto = factory.buildProtocol(('127.0.0.1', 0))
        message = '{{"action": "JOIN_AGENT", "token": "{}" }}'.format(token)
